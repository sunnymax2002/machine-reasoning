...
def donotrun():
    with TypeDB.cloud_driver([SERVER_ADDR], Credentials(USERNAME, PASSWORD), DriverOptions(True, None)) as drv:
        if drv.databases.contains(DB_NAME):
            drv.databases.get(DB_NAME).delete()
        drv.databases.create(DB_NAME)
...
def driver_connect(edition, uri, username=USERNAME, password=PASSWORD):
    ...
    if edition is Edition.Cloud:
        ...
        driver = TypeDB.cloud_driver([uri], Credentials(username, password), DriverOptions(True, None))
